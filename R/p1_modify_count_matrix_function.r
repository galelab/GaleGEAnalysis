...
p1_modify_count_matrix <- function(countfile, targetfile, samples_to_remove_count_matrix=FALSE) {
    ...
    if (typeof(samples_to_remove_count_matrix) == 'character') {
        if ((grep('.txt', samples_to_remove_count_matrix) == 1) || (grep('.csv', samples_to_remove_count_matrix) == 1)) {
            samples_to_remove              <- read.table(samples_to_remove_count_matrix, header=FALSE, row.names=1, check.names = FALSE)
        ...
        if (length(samples_to_remove_count_matrix) > 0) {
            ...
            write.table(data.frame("Name"=rownames(modcounts), modcounts, check.names=FALSE), sep = "\t",col.names=TRUE,  row.names=FALSE, file=file.path(results_path,"count_matrix_mod.txt"))

            write.csv(data.frame("Name"=rownames(modtargets), modtargets, check.names=FALSE), row.names=FALSE, file=file.path(results_path,"targets_mod.csv"))
